...
const onclick = (shape,node) => {
  ...
  shape.items.forEach((item) => {
    ...
    item.click(
      function(e) {
        window.location.href = "rules.html?id="+node.id+"&func="+node.func+"&passid="+node.true_id+"&failid="+node.false_id;
...
const moveover = (shape,node) => {
  const r = shape.paper
  var text = "Rule body \n"+node.func+"\n\n"+"Next rule-id if passed\n\n"+
    node.true_id+"\nNext rule-id if failed\n"+node.false_id;
  shape.items.forEach((item) => {
    item.set = shape;
    if (item.type === 'text') {
      return
    ...
    item.mouseover(
      function(e) {
        this.txt = r.text(e.layerX,e.layerY,text).attr({fill: '#000000'});
      }
    )
    item.mouseout(
      function(e) {
